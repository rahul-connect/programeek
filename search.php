...
$pagination_query = mysqli_query($con,"SELECT id FROM blog WHERE MATCH(title) AGAINST('$search') ORDER BY id DESC");
 ...
 <!-- LEFT SIDEBAR--->
  ...
  <div class="hide-on-med-and-up" style="margin-bottom:20px;">
     <h4 class="center teal-text">Search Results</h4><hr>
	   	...
	   	<?php
	    $fetch_post = mysqli_query($con,"SELECT id,title,category,views,image,date_created FROM blog WHERE MATCH(title) AGAINST('$search') AND status='1' ORDER BY id DESC LIMIT $start,$per_page");
 ...
 <!-- Pagination --->
	 ...
	 <?php
         if($page < 2){
			 ...
			 $back = $page-1;
			 echo "<li class='waves-effect'><a href='search.php?search=$search&page=$back'><i class='material-icons'>chevron_left</i></a></li>";
		 }
		 ...
		 for($i = 1;$i<=$total_pages;$i++){
			 ...
			 echo "<li class='waves-effect'><a href='search.php?search=$search&page=$i'>$i</a></li>";
			 }
		 }
		   ...
		   if($page == $total_pages){
			 ...
			 $next = $page+1;
			 echo "<li class='waves-effect'><a href='search.php?search=$search&page=$next'><i class='material-icons'>chevron_right</i></a></li>";
		 }
